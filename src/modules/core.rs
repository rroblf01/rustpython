...
pub fn create_math_dict() -> HashMap<String, PyObjectRef> {
    ...
    math_func!("tan", |args| {
        ...
        match &*v {
            ...
            PyObject::Float(f) => Ok(py_float(f.tan())),
    ...
    math_func!("exp", |args| {
        ...
        let v = args[0].borrow();
        match &*v {
            PyObject::Int(i) => Ok(py_float(i.to_f64().unwrap_or(0.0).exp())),
            PyObject::Float(f) => Ok(py_float(f.exp())),
            _ => Err(PyError::type_error("exp() argument must be a number")),
        }
    });
    math_func!("pow", |args| {
        if args.len() != 2 {
    ...
    math_func!("acos", |args| {
        ...
        let v = args[0].borrow();
        match &*v {
            PyObject::Int(i) => Ok(py_float(i.to_f64().unwrap_or(0.0).acos())),
            PyObject::Float(f) => Ok(py_float(f.acos())),
            _ => Err(PyError::type_error("acos() argument must be a number")),
        }
    });
    math_func!("fabs", |args| {
        if args.len() != 1 {
    ...
    math_func!("lgamma", |args| {
        ...
        let v = args[0].borrow();
        match &*v {
            PyObject::Int(i) => Ok(py_float(libm::lgamma(i.to_f64().unwrap_or(0.0)))),
            PyObject::Float(f) => Ok(py_float(libm::lgamma(*f))),
            _ => Err(PyError::type_error("lgamma() argument must be a number")),
        }
    });
    math_func!("log2", |args| {
        if args.len() != 1 {
        ...
        let v = args[0].borrow();
        match &*v {
            PyObject::Int(i) => Ok(py_float(i.to_f64().unwrap_or(0.0).log2())),
            PyObject::Float(f) => Ok(py_float(f.log2())),
            _ => Err(PyError::type_error("log2() argument must be a number")),
        }
    });
    ...
    // `math.trunc(x)` calls `x.__trunc__()` when it exists (Fraction etc.),
    // falling back to float truncation otherwise.
    ...
    math_func!("ldexp", |args| {
        ...
        let exp = args[1]
            .as_i64()
            .ok_or_else(|| PyError::type_error("exponent must be an integer"))?;
    ...
    math_func!("fsum", |args| {
        ...
        for item in &items {
            total += math_arg_f64(&item).ok_or_else(|| {
                PyError::type_error(format!(
    ...
    math_func!("sumprod", |args| {
        if args.len() < 2 {
            return Err(PyError::type_error("sumprod() requires 2 arguments"));
        }
        ...
        for (a, b) in p.iter().zip(q.iter()) {
            let av = math_arg_f64(&a)
                .ok_or_else(|| PyError::type_error("sumprod() arguments must be numbers"))?;
            let bv = math_arg_f64(&b)
                .ok_or_else(|| PyError::type_error("sumprod() arguments must be numbers"))?;
            total += av * bv;
        }
        Ok(py_float(total))
    ...
    math_func!("ulp", |args| {
        ...
        if x == 0.0 {
            return Ok(py_float(f64::MIN_POSITIVE));
        }
        let abs = x.abs();
        let next = if abs == f64::INFINITY {
            abs
        } else {
