...
pub(crate) fn operator_compare_digest_builtin(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    let kind = |obj: &PyObjectRef| -> i32 {
        ...
        if matches!(&*effective.borrow(), PyObject::Bytes(_) | PyObject::ByteArray(_)) {
            return 1;
        }
        let is_small = matches!(&effective, PyObjectRef::SmallStr(_));
        let is_str = if is_small { false } else { matches!(&*effective.borrow(), PyObject::Str(_)) };
        if is_small || is_str { 2 } else { 0 }
    };
    ...
    if ka == 2 && kb == 2 {
        ...
        if !sa.is_ascii() || !sb.is_ascii() {
            return Err(PyError::type_error("comparing strings with non-ASCII characters is not supported"));
        }
    ...
    Err(PyError::type_error(format!("unsupported operand types(s) or combination of types: '{}' and '{}'", ta, tb)))
}
